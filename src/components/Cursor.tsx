...
/**
 * A soft ring that trails the pointer and swells over interactive
 * elements. Purely decorative: the native cursor stays visible, and the
 * ring only mounts for fine pointers without a reduced-motion preference.
 */
  ...
  return (
    ...
    >
      <motion.div
        ...
        className={`-ml-4 -mt-4 h-8 w-8 rounded-full border transition-colors duration-300 ${
          active ? "border-accent/70 bg-accent/[0.06]" : "border-white/30"
        }`}
      />
    </motion.div>
  );
